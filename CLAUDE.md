...
- 更新流程：Mac `git bundle create` main → scp 到 Razer `C:\vpdeploy\volo.bundle` → `C:\work\volo` 里 `git fetch origin && git reset --hard origin/main`。前端/vpcal Python 改动即时生效；Rust 改动需重启 App（远程 `schtasks /run /tn VoloDev`，人在机器旁可直接 terminal `pnpm tauri dev`，先 taskkill 旧的 volo/node/vpcal）；vpcal C++ 用 `C:\vpdeploy\build.cmd` 重编。细节见 auto-memory `volo-razer-app-deployment`。
