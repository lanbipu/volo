...
//!
//! The remote script resolves the INTERACTIVE user's profile (driver cache is
//! per-account; the SSH user is uecm-svc, not the production account) and
//! deletes files under the two allowed DXCache roots one-by-one. No path
...
mod tests {
    ...
    fn clear_script_is_single_line_and_deletes_files_only_under_whitelist() {
        ...
        assert!(CLEAR_PS.contains("ProfileList")); // interactive-account resolution
