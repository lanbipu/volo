...
const PROBE_PS: &str = "[Console]::OutputEncoding=[System.Text.Encoding]::UTF8; chcp 65001 | Out-Null; $ErrorActionPreference='Stop'; function Snap($Kind,$Path){ if(Test-Path -LiteralPath $Path){ $files=@(Get-ChildItem -LiteralPath $Path -Recurse -File -Force -ErrorAction SilentlyContinue); $bytes=[int64]0; $newest=$null; foreach($f in $files){ $bytes += [int64]$f.Length; if($null -eq $newest -or $f.LastWriteTimeUtc -gt $newest){ $newest=$f.LastWriteTimeUtc } }; $mtime=$null; if($null -ne $newest){ $mtime=$newest.ToString('o') }; [PSCustomObject]@{ kind=$Kind; path=$Path; exists=$true; file_count=[int64]$files.Count; total_bytes=[int64]$bytes; newest_mtime=$mtime } } else { [PSCustomObject]@{ kind=$Kind; path=$Path; exists=$false; file_count=[int64]0; total_bytes=[int64]0; newest_mtime=$null } } }; $user=(Get-CimInstance Win32_ComputerSystem -ErrorAction SilentlyContinue).UserName; if([string]::IsNullOrWhiteSpace($user)){ throw 'no interactive user logged in - driver cache is per-account, probe must target the production (interactive) account' }; $sid=(New-Object System.Security.Principal.NTAccount($user)).Translate([System.Security.Principal.SecurityIdentifier]).Value; $profile=(Get-ItemProperty \"HKLM:\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\ProfileList\\$sid\").ProfileImagePath; $base=Join-Path $profile 'AppData\\Local'; $local=Join-Path $base 'NVIDIA\\DXCache'; $lowRoot=[System.IO.Path]::GetFullPath((Join-Path $base '..\\LocalLow')); $low=Join-Path $lowRoot 'NVIDIA\\PerDriverVersion\\DXCache'; $boot=(Get-CimInstance Win32_OperatingSystem -ErrorAction SilentlyContinue).LastBootUpTime; $bootTime=$null; if($boot){ $bootTime=([datetime]$boot).ToUniversalTime().ToString('o') }; @{ ok=$true; interactive_user=$user; node_last_boot_time=$bootTime; directories=@(Snap 'local_appdata_dxcache' $local; Snap 'locallow_per_driver_dxcache' $low) } | ConvertTo-Json -Compress -Depth 4";
...
mod tests {
    ...
    fn probe_script_is_single_line_and_mentions_both_cache_dirs() {
        ...
        assert!(PROBE_PS.contains("ProfileList")); // interactive-account resolution
