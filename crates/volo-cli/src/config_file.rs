...
pub fn load(path: &Path) -> VoloResult<FileConfig> {
    #[cfg(unix)]
    {
        use std::os::unix::fs::PermissionsExt;
        let mode = std::fs::metadata(path)
            .map_err(|e| VoloError::Configuration(format!("config {}: {}", path.display(), e)))?
            .permissions()
            .mode();
        if mode & 0o077 != 0 {
    ...
    let text = std::fs::read_to_string(path)
        .map_err(|e| VoloError::Configuration(format!("read config {}: {}", path.display(), e)))?;
    serde_yaml::from_str(&text)
        .map_err(|e| VoloError::Configuration(format!("parse config {}: {}", path.display(), e)))
}
