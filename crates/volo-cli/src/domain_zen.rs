...
use cache_core::core::zen::lua_config::{self, UpstreamInfo};
...
pub(crate) fn default_lifecycle_for(role: &str) -> &'static str {
    match role {
        cache_core::core::zen::endpoint::ROLE_SHARED_UPSTREAM => "installed_service",
        _ => "editor_owned",
    }
}
...
pub(crate) fn run_node(host: &str, script_name: &'static str, args: serde_json::Value) -> UecmResult<String> {
    use cache_core::core::ssh::RemoteExecutor;
    let exec = cache_core::core::ssh::SshExecutor::from_config()?;
    let out = exec.run(
        host,
        &cache_core::core::ssh::NodeScript { name: script_name, args, ssh_user: None },
    )?;
    if out.stdout.trim().is_empty() && out.exit_code != 0 {
        return Err(cache_core::core::ssh::map_exit(out.exit_code, &out.stderr));
    }
    Ok(out.stdout)
}
