...
export interface KeyerParams {
  ...
  wipe: number; despot: number; conditionalStrength: number;
}
export const DEFAULTS: KeyerParams = {
  keyColor: [0.15, 0.6, 0.15], balance: 0.5,
  blackClip: 0.03, whiteClip: 0.95, softness: 1.0, shrink: 0,
  feather: 0, despillStrength: 0.8, despillBalance: 0.5, lumaRestore: 0.5,
  denoise: 0.4, matteStab: 0.5, plateMode: 0, viewMode: 0, wipe: 0.5,
  despot: 0, conditionalStrength: 0,
};
...
export const KNOBS: Knob[] = [
  ...
  { key: "feather",         label: "羽化",         min: 0,   max: 1.5, step: 0.05 },
  { key: "despot",          label: "去孤点",       min: 0,   max: 1, step: 0.01 },
...
export function packParams(p: KeyerParams): Float32Array {
  return new Float32Array([
    ...
    p.wipe, p.despot, p.conditionalStrength, 0,
  ]);
}
