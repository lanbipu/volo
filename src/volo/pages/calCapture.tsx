  ...
  function CaptureModal({ s, close }) {
    ...
    const body = h('div', { className: 'drawer-b' },
      ...
      h('div', { className: 'cap-card' },
        ...
        form.inverted && !form.patternsDir ? h('div', { className: 'vs-tf-note', style: { marginTop: 10, color: 'var(--notice-visual)' } },
          'inverted=true 需要在「采集参数」里设置 patternsDir（normal.png / inverted.png 所在目录），否则不能保存；不用差分采集可关闭 inverted。') : null),
      h('div', { className: 'cap-card' },
